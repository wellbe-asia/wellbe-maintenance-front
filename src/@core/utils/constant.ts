export const SERVER_COMMON_URL = `http://localhost:8082`
export const SERVER_SHOP_URL = `http://localhost:8081`
export const API_KEY_REQUEST_HEADER_NAME = 'Wellbe_ShopToken'
export const API_KEY_WITH_LOGIN = `XyJkXBw4QnpJHnmK4KtVsRoMGkBqoD5`
export const API_KEY_WITHOUT_LOGIN = `fpbaVPtY4c5ayApcztyfMrFdhmmHAYA`
export const API_SHOP_TOKEN_REQUEST_HEADER_NAME = 'Wellbe_ShopToken'

export const APP_NAME = `conduit`

export const ARTICLE_QUERY_MAP = {
  'tab=feed': `${SERVER_COMMON_URL}/articles/feed`,
  'tab=tag': `${SERVER_COMMON_URL}/articles/tag`
}

export const DEFAULT_PROFILE_IMAGE = `https://static.productionready.io/images/smiley-cyrus.jpg`

export const DEFAULT_LIMIT = 20

export const DEFAULT_IMAGE_SOURCE = `data:image/png;base64,`

export const SESSION_STORAGE_KEY_KEYWORD = {
  ACCOUNTID: 'wellbeMaintenanceAccountId',
  LANGUAGE: 'wellbeLanguage',
  TOKEN: 'wellbeToken'
}

export const DEFAULT_SESSION_STORAGE = {
  ACCOUNTID: 'DEFAULT',
  LANGUAGE: 'DEFAULT',
  TOKEN: 'DEFAULT'
}

export const REGEX_TIME = /^[0-9][0-9]:[0-9][0-9]$/

export const SHOP_CONTENT_CATEGORY = {
  MAIN_DESCRIPTION: '1'
}

export const IMAGE_CATEGORY = {
  MAIN_IMAGE: '1'
}

export const SHOP_EQUIPMENT = {
  BED: '3'
}

export const EXTERNAL_SITE_CD = {
  SALONBOARD: '1'
}

export const EXTERNAL_SITE_SALONBOARD_CONFIG_VALUE = {
  HAIRE: '1',
  KIREI: '2'
}

export const SHOP_STATUS = {
  UNAPPROVED: '1',
  APPROVED: '2',
  SUSPENDING: '3',
  CLOSED: '4',
  REQUESTED: '5'
}

export const BOOKING_STATUS = {
  REQUEST: '1',
  FIXED: '2',
  CANCELD: '3'
}

export const BOOKING_STATUS_FILTER_NAME = {
  REQUEST: 'Request',
  FIXED: 'Fixed',
  CANCELD: 'Canceled'
}

export const GENDER = {
  MAN: '1',
  WOMAN: '2'
}

export const BOOKING_CHANEL = {
  WELLBE: '1',
  DIRECT: '2'
}

export const DEFAULT_SORT_ORDER = {
  FIRST: '1'
}

export const SHOP_MAINTENANCE_LABEL = {
  INDIVIDUAL_PAYOUT: '1'
}

export const YES_NO = {
  NO: '0',
  YES: '1'
}

export const REVIEW_STATUS = {
  NOT_POSTED: '1',
  NOT_APPROVED: '2',
  APPROVED: '3',
  DENIED: '4',
  DELETED: '5'
}

export const CURRENCY = {
  USD: '1',
  JPY: '2',
  VND: '3'
}

export const INVOICE_STATUS = {
  UNCRAIMED: '1',
  BILLED: '2',
  DEPOSITED: '3',
  WITHDRAWAL: '4'
}

export const COUPON_TARGET_ATTR_CD = {
  ALL_CUSTOMER: '1',
  REPEAT_CUSTOMER: '2',
  NEW_CUSTOMER: '3'
}

export const COUPON_TARGET_ATTR_VALUE = {
  FALSE: '0',
  TRUE: '1'
}

export const SERVER_STATUS = {
  SUCCESS: 0,
  CANNOT_SIGNUP: 401,
  CANNOT_LOGIN: 402,
  TOKEN_PARSE_ERROR: 403,
  CANNOT_CONFIRM_SIGNUP: 404,
  TOKEN_NOTEXISTS: 405,
  APIKEY_MISMATCH: 406,
  NEED_LOGIN: 407,
  TOKEN_ID_MISMATCH_WITH_KEY: 408,
  APIKEY_NOTMATCH: 409,
  EMAIL_DUPPLICATE: 410,
  EMAIL_ISNOTEXISTS: 411,
  CANNOT_CHANGE_EMAIL: 412,
  CANNOT_CHANGE_PASSWORD: 413,
  URL_DUPPLICATE: 414,
  LOGIC_ERROR_CODE_SHOP_URL_NOT_REGISTERED: 415,
  LOGIC_ERROR_CODE_CONTENTS_URL_DUPPLICATE: 416,
  LOGIC_ERROR_CODE_CHECKOUT_TIMING_NOT_REGISTERED: 417,
  LOGIC_ERROR_CODE_SHOP_NOTEXISTS_EXTERNAL_CONNECTION: 419,
  NOTEXISTS_UPDATE: 501,
  NOTEXISTS_DELETE: 502,
  NOTEXISTS_FOREIGN_KEY: 511,
  NOTALLOWED_MENU_FOR_MALE: 601,
  NOTALLOWED_MENU_FOR_FEMALE: 602,
  BOOKING_NOT_AVAILABLE: 603,
  BOOKING_ID_DUPPLICATE: 604,
  ACCOUNT_IS_TEMPORARY: 605,
  ACCOUNT_IS_WITHDRAWN: 606,
  SHOP_LOCATION_NONAVAILABLE: 607,
  CANCEL_LIMIT_EXPIRED: 608,
  PAYMENT_REFUND_NONAVAILABLE: 609,
  BOOKING_LIMIT_EXPIRED: 610,
  BOOKING_ALREADY_CANCELED: 611,
  LOGIC_ERROR_CODE_BOOKING_IS_NOT_EXISTS: 612,
  LOGIC_ERROR_CODE_CHECKOUT_ALREADY: 613,
  LOGIC_ERROR_CODE_BOOKING_STATUS_IS_REQUEST: 614,
  LOGIC_ERROR_CODE_BOOKING_STATUS_IS_CANCEL: 615,
  PAYPAL_CHECKOUT_DENIED: 701,
  PAYPAL_NOT_CHECKOUT_YET: 702,
  PAYPAL_REFUND_DENIED: 703,
  DBERROR: 901,
  DATETIME_PARSEERROR: 902,
  FILEIOERROR: 903,
  ENCODEERROR: 904,
  MAILERERROR: 905,
  ENVERROR: 906,
  NUMBERING_OVERFLOW: 907,
  NUMBERING_NOTEXISTS: 908,
  JWK_NOTEXISTS: 909,
  PAYPAL_APIERROR: 910,
  MASTER_DATA_IS_UNSETUP: 911,
  COMPARE_STRING_INVALID: 912,
  MAIERERROR: 913,
  EXCHANGE_PAIRE_IS_UNSETUP: 914,
  SEVERERROR: 990
}

export const DEFAULT_DESCRIPTION_LANGUAGE = '1'
export const DEFAULT_LANGUAGE = '1'
